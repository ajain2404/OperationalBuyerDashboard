sap.ui.define([
	...
	return sap.ui.controller("com.evonik.sc.purchasing.operationalbuyerovp.ext.custom", {
		...
		*/
		...
		onCustomActionPress: function (sCustomAction) {
			...
			if (sCustomAction === "releasePurchaseOrder") {
				return this.releasePurchaseOrder;
			} else if (sCustomAction === "rejectPurchaseOrder") {
				return this.rejectPurchaseOrder;
			} else if (sCustomAction === "forwardPurchaseOrder") {
				return this.forwardPurchaseOrder;
			}

			return this.actionNotImplemented;
		},

		releasePurchaseOrder: function (oEvent) {
			sap.m.MessageToast.show("releasePurchaseOrder");
		},
		rejectPurchaseOrder: function (oEvent) {
			sap.m.MessageToast.show("rejectPurchaseOrder");
		},
		forwardPurchaseOrder: function (oEvent) {
			sap.m.MessageToast.show("forwardPurchaseOrder");
		},
		...
		*/
		onCustomParams: function (sCustomParams) {
			if (sCustomParams === "getOpenPRParams") {
				return this.getOpenPRParams.bind(this);
			}
			return null;
		},
		getOpenPRParams: function (oNavigateParams, oSelectionVariantParams) {

			return this._oCockpitUtility.mapSelectionParameters(this, this._aParameterMapping, oSelectionVariantParams);

			// //oSelectionVariantParams is not populated in version 1.52
			// if (oSelectionVariantParams) {

			// 	for (var i = 0; i < this._aParameterMapping.length; i++) {

			// 		var oSelectOption = oSelectionVariantParams.getSelectOption(this._aParameterMapping[i].sourceparameter);
			// 		if (!oSelectOption) {
			// 			continue;
			// 		}
			// 		for (var j = 0; j < oSelectOption.length; j++) {
			// 			oFilter = {
			// 				path: this._aParameterMapping[i].targetparameter,
			// 				operator: oSelectOption[j].Option,
			// 				value1: oSelectOption[j].Low,
			// 				value2: oSelectOption[j].High,
			// 				sign: oSelectOption[j].Sign
			// 			};
			...
			// 	var oSelectionVariant = oGlobalFilter.getUiState().getSelectionVariant();
			// 	if (!oSelectionVariant || !oSelectionVariant.SelectOptions || oSelectionVariant.SelectOptions.length === 0) {
			// 		return;
			// 	}

			// 	for (i = 0; i < this._aParameterMapping.length; i++) {
			// 		for (j = 0; j < oSelectionVariant.SelectOptions.length; j++) {
			// 			if (this._aParameterMapping[i].sourceparameter === oSelectionVariant.SelectOptions[j].PropertyName) {
			// 				for (var k = 0; k < oSelectionVariant.SelectOptions[j].Ranges.length; k++) {
			// 					oFilter = {
			// 						path: this._aParameterMapping[i].targetparameter,
			// 						operator: oSelectionVariant.SelectOptions[j].Ranges[k].Option,
			// 						value1: oSelectionVariant.SelectOptions[j].Ranges[k].Low,
			// 						value2: oSelectionVariant.SelectOptions[j].Ranges[k].High,
			// 						sign: oSelectionVariant.SelectOptions[j].Ranges[k].Sign
			// 					};
			// 					aCustomSelectionVariant.push(oFilter);
			// 				}
			// 			}
